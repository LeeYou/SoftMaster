...
int SoftUnstallAdapter::getCount()
{
	return m_softInfo.GetCount()*NUMSCALE;
}

SStringT SoftUnstallAdapter::getSizeText(DWORD dwSize)
{
...
void SoftUnstallAdapter::getView(int position, SWindow * pItem, pugi::xml_node xmlTemplate)
{
	...
	pBtnFav->GetEventSet()->subscribeEvent(EVT_CMD,
		Subscriber(&SoftUnstallAdapter::OnButtonClick, this));*/
}

bool SoftUnstallAdapter::OnButtonClick(EventArgs *pEvt)
{
...
//删除一行，提供外部调用。
void SoftUnstallAdapter::DeleteItem(int iPosition)
{
...
SStringW SoftUnstallAdapter::GetColumnName(int iCol) const {
	return SStringW().Format(L"col%d", iCol + 1);
}

bool SoftUnstallAdapter::OnSort(int iCol, SHDSORTFLAG * stFlags, int nCols)
{
...
int __cdecl SoftUnstallAdapter::SortCmp(void *context, const void * p1, const void * p2)
{
