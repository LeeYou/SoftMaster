...
CustomButton::CustomButton(void) :m_ptPushOffet(2, 2), m_drawMode(HORZ_ICON_TEXT), m_nSepSize(5), m_hIcon(0), m_nIconSize(32), m_iIcon(0), m_bIconVisible(true), m_pIcon(NULL)
{
	m_bClipClient = TRUE;
}
...
CSize CustomButton::GetDesiredSize(LPCRECT pRcContainer)
{
...
CSize CustomButton::GetDesiredSize(IRenderTarget *pRT, LPCRECT pRcContainer)
{
	SASSERT(m_pIcon);
...
void CustomButton::OnPaint(IRenderTarget *pRT)
{
	SPainter painter;
...
HICON CustomButton::SetIcon(HICON hIcon, int nIconSize)
{
	HICON hRet = m_hIcon;
	m_hIcon = hIcon;
...
CSize CustomButton::GetIconSize()
{
	if (m_hIcon) return CSize(m_nIconSize, m_nIconSize);
	else return m_pIcon->GetSkinSize();
}

void CustomButton::DrawIcon(IRenderTarget *pRT, CRect rcIcon)
{
...
void CustomButton::SetIconVisible(bool bVisible)
{
